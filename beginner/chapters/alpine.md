...
The `pull` command fetches the alpine **image** from the **[Docker Hub](https://hub.docker.com/_/alpine)** registry and saves it in our system. You can use the `docker images` command to see a list of all images on your system.
```
$ docker images
...
2. The Docker daemon checks local store if the image (alpine in this case) is available locally, and if not, downloads it from **[Docker Hub](https://hub.docker.com/)**. (Since we have issued `docker pull alpine` before, the download step is not necessary)
