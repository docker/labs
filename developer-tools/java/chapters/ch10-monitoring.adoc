...
Docker exposes Prometheus-compatible metrics on port `9323`. This support is only available as experimental.
...
In this section, we'll start Prometheus node exporter and scrape its own health.

Let's start Prometheus node exporter
...
```
+
This configuration file scrapes data from two targets:
...
+
This output shows that the metrics exposed by Prometheus container are configured correctly. But, the metrics for Docker Engine are not scraped. This is tracked as https://github.com/docker/labs/issues/344.
+
...
cAdvisor also exposes container statistics as Prometheus metrics out of the box. By default, these metrics are served under the `/metrics` HTTP endpoint. The following configuration fragment can be added to `prometheus.yml` to scrape the metrics:
