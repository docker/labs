...
`build` command is also passed a context that is used during image creation. This context can be a path to your local filesystem or a URL to a Git repository.

_Dockerfile_ is usually called _Automate Building of Images_. The complete list of commands that can be specified in this file are explained at https://docs.docker.com/reference/builder/. The common commands are listed below:
