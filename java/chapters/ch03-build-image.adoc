...
==== Package Java Application using Docker Gradle Plugin

The https://github.com/Transmode/gradle-docker[Gradle Docker Plugin] is an alternative to the Maven Docker Plugin for Gradle users. Add the following configuration to your Gradle file:
...
The new task `buildDocker` will build the jar file and the docker image, named with the `applicationName`.
